...
| 화면 | 설명 | 상태 |
|------|------|------|
| HomeScreen | 거래 목록, 잔액 요약, 필터링 | 완료 |
| AddTransactionScreen | 거래 추가 폼 | 완료 |
| SettingsScreen | 카테고리/결제수단 관리 (탭 전환) | 완료 |

### 3.1 향후 화면 구조 (Phase 2+)
...
### Phase 1: MVP (완료)
- [x] 프로젝트 설정
- [x] 서비스 레이어 구현 (TDD, 100% 커버리지)
  - CategoryService
  - PaymentMethodService
  - TransactionService
- [x] 기본 UI 컴포넌트
  - SummaryCard (잔액/수입/지출 요약)
  - TransactionItem (거래 항목)
- [x] 리스트 뷰 (HomeScreen)
- [x] 거래 CRUD
- [x] UI 테스트 (137개 테스트 통과)

### Phase 1.5: UI 완성 (진행 중)
- [x] 캘린더 뷰
- [x] 통계 탭 (DonutChart, GroupedBarChart)
- [ ] 테마 (Light/Dark)

### Phase 2: 데이터 영속성 (완료)
> 참고: [데이터 아키텍처 설계](data-architecture.md)
...
- [x] 앱 시작 시 데이터 로드
- [x] 테스트 코드 업데이트 (612개 테스트, 95.11% 커버리지)

### Phase 3: Google Sheets 동기화
> **주의**: Google Sheets API 제한사항 (60회/분/사용자)으로 인해 실시간 동기화 대신 수동 백업/복원 방식 채택
> 참고: [데이터 아키텍처 설계](data-architecture.md)
